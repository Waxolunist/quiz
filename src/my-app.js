...
requestAnimationFrame(function(){self.fire("app-reset-layout")})},_notifyDescendantResize:function _notifyDescendantResize(){if(!this.isAttached){return}this._interestedResizables.forEach(function(resizable){if(this.resizerShouldNotify(resizable)){this._notifyDescendant(resizable)}},this)}}];_exports.AppLayoutBehavior=AppLayoutBehavior;var appLayoutBehavior={AppLayoutBehavior:AppLayoutBehavior};_exports.$appLayoutBehavior=appLayoutBehavior;Polymer({_template:html(_templateObject2_1a609510971711e9a12f3787802a8791()),is:"app-header-layout",behaviors:[AppLayoutBehavior],properties:{/**
...
if(30<Math.abs(this._initScrollTop-scrollTop)||10<absDScrollTop){if(isScrollingDown&&scrollTop>=maxHeaderTop){top=maxHeaderTop}else if(!isScrollingDown&&scrollTop>=this._dHeight){top=this.condenses&&!this.fixed?this._dHeight:0}var scrollVelocity=dScrollTop/(now-this._lastTimestamp);this.style.transitionDuration=this._clamp((top-lastTop)/scrollVelocity,0,300)+"ms"}else{top=this._top}}}if(0===this._dHeight){progress=0<scrollTop?1:0}else{progress=top/this._dHeight}if(!forceUpdate){this._lastScrollTop=scrollTop;this._top=top;this._wasScrollingDown=isScrollingDown;this._lastTimestamp=now}if(forceUpdate||progress!==this._progress||lastTop!==top||0===scrollTop){this._progress=progress;this._runEffects(progress,top);this._transformHeader(top)}},/**
   ...
   * @this Polymer.AppLayout.ElementWithBackground
   */setUp:function setUp(config){var title=this._getDOMRef("mainTitle"),condensedTitle=this._getDOMRef("condensedTitle"),duration=config.duration||"0.2s",fx={};if(!condensedTitle){console.warn("Scroll effect `resize-snapped-title`: undefined `condensed-title`");return!1}if(!title){console.warn("Scroll effect `resize-snapped-title`: undefined `main-title`");return!1}title.style.transitionProperty="opacity";title.style.transitionDuration=duration;condensedTitle.style.transitionProperty="opacity";condensedTitle.style.transitionDuration=duration;fx.condensedTitle=condensedTitle;fx.title=title;this._fxResizeSnappedTitle=fx},/** @this Polymer.AppLayout.ElementWithBackground */run:function run(p,y){var fx=this._fxResizeSnappedTitle;if(0<p){fx.title.style.opacity=0;fx.condensedTitle.style.opacity=1}else{fx.title.style.opacity=1;fx.condensedTitle.style.opacity=0}},/** @this Polymer.AppLayout.ElementWithBackground */tearDown:function tearDown(){var fx=this._fxResizeSnappedTitle;fx.title.style.transition="";fx.condensedTitle.style.transition="";delete this._fxResizeSnappedTitle}});Polymer({_template:html(_templateObject4_1a609510971711e9a12f3787802a8791()),is:"app-toolbar"});Polymer({is:"iron-location",_template:null,properties:{/**
...
if(setObj.data!==void 0){this._pathEffector("data",this.data);this._notifyChange("data")}if(setObj.active!==void 0){this._pathEffector("active",this.active);this._notifyChange("active")}if(setObj.tail!==void 0){this._pathEffector("tail",this.tail);this._notifyChange("tail")}}});var IronMeta=/*#__PURE__*/function(){/**
   * @param {{
...
IronMeta[" "]=function(){};IronMeta.types={};var metaDatas=IronMeta.types;/**
   ...
   * @return {*}
   */byKey:function byKey(key){return new IronMeta({type:this.type,key:key}).value}});var ironMeta={IronMeta:IronMeta};_exports.$ironMeta=ironMeta;Polymer({_template:html(_templateObject6_1a609510971711e9a12f3787802a8791()),is:"iron-icon",properties:{/**
...
switch(page){case"scanner":new Promise(function(res,rej){return _require.default(["./scanner.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$scanner||{}});break;case"solution":new Promise(function(res,rej){return _require.default(["./solution.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$solution||{}});break;case"question":new Promise(function(res,rej){return _require.default(["./question.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$question||{}});break;case"nexthint":new Promise(function(res,rej){return _require.default(["./nexthint.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$nexthint||{}});break;case"view404":new Promise(function(res,rej){return _require.default(["./my-view404.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$myView404||{}});break;}}}],[{key:"template",get:function get(){return html(_templateObject9_1a609510971711e9a12f3787802a8791())}},{key:"properties",get:function get(){return{page:{type:String,reflectToAttribute:!0,observer:"_pageChanged"},routeData:Object,subroute:Object}}},{key:"observers",get:function get(){return["_routePageChanged(routeData.page)"]}}]);return MyApp}(PolymerElement);window.customElements.define("my-app",MyApp);var $_documentContainer$1=document.createElement("template");$_documentContainer$1.innerHTML="<dom-module id=\"shared-styles\">\n  <template>\n    <style>\n      .card {\n        height: calc(100% - 10px);\n        box-sizing: border-box;\n        margin: 5px;\n        padding: 15px;\n        color: #757575;\n        border-radius: 5px;\n        background-color: #fff;\n        box-shadow: 0 2px 2px 0 rgba(0, 0, 0, 0.14), 0 1px 5px 0 rgba(0, 0, 0, 0.12), 0 3px 1px -2px rgba(0, 0, 0, 0.2);\n        display: flex;\n        flex-direction: column;\n        overflow-x: auto;\n      }\n\n      .card > h1,\n      .card > .header {\n        flex-grow: 0;\n      }\n\n      .card > * {\n        flex-grow: 1;\n      }\n\n      .circle {\n        display: inline-block;\n        width: 64px;\n        height: 64px;\n        text-align: center;\n        color: #555;\n        border-radius: 50%;\n        background: #ddd;\n        font-size: 30px;\n        line-height: 64px;\n        flex-grow: 0;\n        margin-right: 10px;\n      }\n\n      .header > * {\n        display: inline-block;\n      }\n\n      h1 {\n        margin: 16px 0;\n        color: #212121;\n        font-size: 22px;\n      }\n    </style>\n  </template>\n</dom-module>";document.head.appendChild($_documentContainer$1.content);var QuestionViewElement=/*#__PURE__*/function(_PolymerElement3){babelHelpers.inherits(QuestionViewElement,_PolymerElement3);function QuestionViewElement(){babelHelpers.classCallCheck(this,QuestionViewElement);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(QuestionViewElement).apply(this,arguments))}babelHelpers.createClass(QuestionViewElement,[{key:"_questionChanged",value:function _questionChanged(q){var qObj=localStorage.getItem(q);if(qObj){this.question=JSON.parse(qObj);if(this._onQuestionLoaded){this._onQuestionLoaded(this.question)}}return q}},{key:"_playsound",value:function _playsound(name,cb){var sound=new Howl({src:["".concat(this.rootPath,"assets/sounds/").concat(name,".mp3")]});if(cb){sound.on("end",function(event){return setTimeout(function(e){return cb(event)},100)})}sound.play()}},{key:"_changePath",value:function _changePath(newpath){this.dispatchEvent(new CustomEvent("viewchange",{detail:{path:newpath},bubbles:!0,composed:!0}))}},{key:"_viewActivated",value:function _viewActivated(active){if(this._onViewActivated){this._onViewActivated()}}}],[{key:"properties",get:function get(){return{questionId:{type:String,observer:"_questionChanged"},question:Object,active:{type:Boolean,observer:"_viewActivated"}}}}]);return QuestionViewElement}(PolymerElement);_exports.QuestionViewElement=QuestionViewElement;var viewElement={QuestionViewElement:QuestionViewElement};_exports.$viewElement=viewElement});