...
if(setObj.data!==void 0){this._pathEffector("data",this.data);this._notifyChange("data")}if(setObj.active!==void 0){this._pathEffector("active",this.active);this._notifyChange("active")}if(setObj.tail!==void 0){this._pathEffector("tail",this.tail);this._notifyChange("tail")}}});class IronMeta{/**
   * @param {{
...
activateEvent:{type:String,value:null}},observers:["_selectedPageChanged(selected)"],_selectedPageChanged:function(selected,old){this.async(this.notifyResize)}});const template$1=html`
<custom-style>
  <style is="custom-style">
    html {
...
`;template$1.setAttribute("style","display: none;");document.head.appendChild(template$1.content);const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`<iron-iconset-svg name="my-icons" size="24">
  <svg>
    <defs>
      <g id="arrow-back">
...
setRootPath(MyAppGlobals.rootPath);class MyApp extends PolymerElement{static get template(){return html`
    ...
    `}static get properties(){return{page:{type:String,reflectToAttribute:!0,observer:"_pageChanged"},routeData:Object,subroute:Object}}static get observers(){return["_routePageChanged(routeData.page)"]}ready(){super.ready();new URL(document.querySelector("base").href).pathname;fetch("./assets/questions.json").then(function(response){return response.json()}).then(function(response){response.questions.forEach(q=>{localStorage.setItem(q.id,JSON.stringify(q))})});fetch("./assets/questions-html.json").then(function(response){return response.json()}).then(function(response){Object.entries(response).forEach(array=>{localStorage.setItem("template-"+array[0],array[1])})})}connectedCallback(){super.connectedCallback();this.addEventListener("viewchange",this._routeChange)}disconnectedCallback(){super.disconnectedCallback();window.removeEventListener("viewchange",this._routeChange)}_routeChange(e){//this.set('route.path', this.rootPath + e.detail.path);
this.set("route.path","/"+e.detail.path)}_routePageChanged(page){// Show the corresponding page according to the route.
//
// If no page was found in the route data, page will be an empty string.
// Show 'view1' in that case. And if the page doesn't exist, show 'view404'.
if(!page){this.page="scanner"}else if(-1!==["scanner","question","solution","nexthint"].indexOf(page)){this.page=page}else{this.page="view404"}}_pageChanged(page){// Import the page component on demand.
//
// Note: `polymer build` doesn't like string concatenation in the import
// statement, so break it up.
switch(page){case"scanner":new Promise((res,rej)=>_require.default(["./scanner.js"],res,rej)).then(bundle=>bundle&&bundle.$scanner||{});break;case"solution":new Promise((res,rej)=>_require.default(["./solution.js"],res,rej)).then(bundle=>bundle&&bundle.$solution||{});break;case"question":new Promise((res,rej)=>_require.default(["./question.js"],res,rej)).then(bundle=>bundle&&bundle.$question||{});break;case"nexthint":new Promise((res,rej)=>_require.default(["./nexthint.js"],res,rej)).then(bundle=>bundle&&bundle.$nexthint||{});break;case"view404":new Promise((res,rej)=>_require.default(["./my-view404.js"],res,rej)).then(bundle=>bundle&&bundle.$myView404||{});break;}}}window.customElements.define("my-app",MyApp);const $_documentContainer$1=document.createElement("template");$_documentContainer$1.innerHTML=`<dom-module id="shared-styles">
  <template>
    <style>
      .card {
...
</dom-module>`;document.head.appendChild($_documentContainer$1.content);class QuestionViewElement extends PolymerElement{static get properties(){return{questionId:{type:String,observer:"_questionChanged"},question:Object,active:{type:Boolean,observer:"_viewActivated"}}}_questionChanged(q){let qObj=localStorage.getItem(q);if(qObj){this.question=JSON.parse(qObj);if(this._onQuestionLoaded){this._onQuestionLoaded(this.question)}}return q}_playsound(name,cb){var sound=new Howl({src:[`${this.rootPath}assets/sounds/${name}.mp3`]});if(cb){sound.on("end",event=>setTimeout(e=>cb(event),100))}sound.play()}_changePath(newpath){this.dispatchEvent(new CustomEvent("viewchange",{detail:{path:newpath},bubbles:!0,composed:!0}))}_viewActivated(active){if(this._onViewActivated){this._onViewActivated()}}}_exports.QuestionViewElement=QuestionViewElement;var viewElement={QuestionViewElement:QuestionViewElement};_exports.$viewElement=viewElement});